...
app.get('/', (req, res) => {
  res.json({ status: 'MarketEdge Pro Server v4.1', endpoints: ['/prices', '/quote?tickers=X', 'POST /ai'] });
});

// GET /indices — BSE Sensex, NSE Nifty, Bank Nifty, India VIX
const INDEX_SYMBOLS = {
  SENSEX:   '^BSESN',
  NIFTY:    '^NSEI',
  BANKNIFTY:'^NSEBANK',
...
app.listen(PORT, () => console.log(`MarketEdge Pro Server v4.1 on port ${PORT}`));
